...
# transformer
class Transformer_ViT16(nn.Module):
    def __init__(self):
        super().__init__()
        
        # Load DINO model        
        self.dino_model = torch.hub.load('facebookresearch/dino:main', 'dino_vitb16', pretrained=True)
    ...
    def forward(self, x, reference_locations):
        ...
        # Cross attention
        cross_attention_layer_1, attn_map_1, q1, k1, v1 = self.cross_attention_1(context_embeddings_1.permute(0, 2, 3, 1).reshape(B, -1, 256), reference_locations)
        cross_attention_layer_2, attn_map_2, q2, k2, v2 = self.cross_attention_2(context_embeddings_2.permute(0, 2, 3, 1).reshape(B, -1, 256), reference_locations)
        cross_attention_layer_3, attn_map_3, q3, k3, v3 = self.cross_attention_3(context_embeddings_3.permute(0, 2, 3, 1).reshape(B, -1, 256), reference_locations)
        cross_attention_layer_4, attn_map_4, q4, k4, v4 = self.cross_attention_4(context_embeddings_4.permute(0, 2, 3, 1).reshape(B, -1, 256), reference_locations)
