...
import material_prediction.materialistic.src.models.model_utils as model_utils
import material_prediction.materialistic.src.loss_functions.loss_utils as loss_utils
import material_prediction.materialistic.src.utils.utils as utils
...
class Transformer_pl(pl.LightningModule):
    ...
    def load_checkpoint(self, checkpoint_path, map_location=None):
        ...
        if '.ckpt' in files[-1]:
            ...
            self.load_state_dict(torch.load(os.path.join(checkpoint_path, files[-1]), map_location= map_location if torch.cuda.is_available() else torch.device('cpu'))['state_dict'])
        else:
